...
public class CalendarUtils {
    ...
    public static class TimeZoneUtils {
        ...
        public static final String[] CALENDAR_CACHE_POJECTION = { CalendarCache.KEY, CalendarCache.VALUE };
         ...
         * 
         * If the {@link DateUtils#FORMAT_UTC} flag is used it will pass in the
         ...
         * 
         * Updates the home time zone in the db asynchronously and updates the
         * local cache. Sending a time zone of
         * {@link CalendarCache#TIMEZONE_TYPE_AUTO} will cause it to be set to
         * the device's time zone. null or empty tz will be ignored.
         * 
         * @param context
         *            The calling activity
         * @param timeZone
         *            The time zone to set Calendar to, or
        ...
        public void setTimeZone(Context context, String timeZone) {
            ...
            if (updatePrefs) {
                ...
                mHandler.startUpdate(mToken, null, CalendarCache.URI, values, "key=?", TIMEZONE_TYPE_ARGS);

                // If using a home tz write it to the db
                if (mUseHomeTZ) {
                    ContentValues values2 = new ContentValues();
                    values2.put(CalendarCache.VALUE, mHomeTZ);
                    mHandler.startUpdate(mToken, null, CalendarCache.URI, values2, "key=?", TIMEZONE_INSTANCES_ARGS);
                }
            }
        }
         ...
         * 
         * This is a helper method to get the appropriate time zone for
         * Calendar. If this is the first time this method has been called it
         * will initiate an asynchronous query to verify that the data in
         * preferences is correct. The callback supplied will only be called if
         * this query returns a value other than what is stored in preferences
         * and should cause the calling activity to refresh anything that
         * depends on calling this method.
         * 
         * @param context
         *            The calling activity
         * @param callback
         *            The runnable that should execute if a query returns new
         *            values
        ...
        public String getTimeZone(Context context, Runnable callback) {
            synchronized (mTZCallbacks) {
                if (mFirstTZRequest) {
                    ...
                    mHandler.startQuery(0, context, CalendarCache.URI, CALENDAR_CACHE_POJECTION, null, null, null);
                }
     ...
     * 
     * @param context
     *            A context with access to the correct preferences
     * @param key
     *            The preference to write to
     * @param value
     *            The value to write
     */
